...
class Cake{
	public int size; // 케이크 사이즈
	public Cake(int sz) {
		size = sz;
	}
	public void showCakeSize() {
		System.out.println();
		System.out.println("Bread ounces : " + size);  //오버라이딩 되면 상위 클래스의 메서드는 덮어씌워짐.
	}
}

class CheeseCake extends Cake {
	public int size; // 치즈케이크 사이즈
	...
	public void showCakeSize() {
		...
		// size는 이 클래스 CheeseCake의 멤버 size를 의미
...
public class P_341_342 {
	public static void main(String[] args) {
		...
		//ca2는 Cake 형태 이므로 ca2.size는 Cake의 멤버 size를 의미함
		System.out.println("Bread Ounces : " + ca2.size);
		
		//ca1은 CheeseCake 형태이므로 ca1.size는 CheeseCake의 멤버 size를 의미함
